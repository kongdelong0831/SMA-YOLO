...
class LoadImages:
    ...
    def __next__(self):
        ...
        if self.video_flag[self.count]:
            ...
            f_num = len(os.listdir(os.path.dirname(path)))
            f_index = os.path.basename(path).split(".")[0][-6:]
            f_1 = path.replace(f_index, f"{max(int(f_index)-1,1):06d}")
            f_2 = path
            f_3 = path.replace(f_index, f"{min(int(f_index)+1,f_num):06d}")
            im1 = cv2.imread(f_1)  # BGR
            im2 = cv2.imread(f_2)
            im3 = cv2.imread(f_3)
            im0 = np.concatenate([im1, im2, im3], axis=2)
            if im0 is None:
