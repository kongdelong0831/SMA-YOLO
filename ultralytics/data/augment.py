        ...
        if self.hgain or self.sgain or self.vgain:
            ...
            if img.shape[2] != 3:
                ...
                for img in imgs:
                    ...
                    imgs_.append(img_bgr)
                img = np.concatenate(imgs_, axis=2)
                labels["img"] = img
            else:
                hue, sat, val = cv2.split(cv2.cvtColor(img, cv2.COLOR_BGR2HSV))
                dtype = img.dtype  # uint8
        ...
        if img.shape[2] == 3:
            ...
            for img_ in imgs:
                img_ = cv2.copyMakeBorder(
                    img_, top, bottom, left, right, cv2.BORDER_CONSTANT, value=(114, 114, 114)
                )
                imgs_.append(img_)
            img = np.concatenate(imgs_, axis=2)
    ...
    def __call__(self, labels):
        ...
        if len(cls):
            ...
            if self.transform and random.random() < self.p:
                if im.shape[2] != 3:
                    imgs = [im[:, :, :3], im[:, :, 3:6], im[:, :, 6:9]]
                    imgs_ = []
                    for img in range(3):
                        new = self.transform(image=imgs[img], bboxes=bboxes, class_labels=cls)
                        imgs_.append(new["image"])
                        if img == 1:
                            class_labels = new["class_labels"]
                            bboxes = new["bboxes"]
                    new["class_labels"] = class_labels
                    new["bboxes"] = bboxes
                    new["image"] = np.concatenate(imgs_, axis=2)
                else:
