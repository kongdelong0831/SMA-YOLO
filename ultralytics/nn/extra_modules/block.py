...
__all__ = ['DyHeadBlock', 'DyHeadBlockWithDCNV3', 'Fusion', 'C2f_Faster', 'C3_Faster', 'C3_ODConv', 'C2f_ODConv', 'Partial_conv3', 'C2f_Faster_EMA', 'C3_Faster_EMA', 'C2f_DBB',
           ...
           'C3_SMPCGLU', 'C2f_SMPCGLU', 'C3_Heat', 'C2f_Heat', 'PSA', 'SBA', 'WaveletPool', 'WaveletUnPool', 'CSP_PTB', 'GLSA', 'CSPOmniKernel', 'MDFormer']
...
class DWConv(nn.Module):
    def __init__(self, in_channels, kernel_size):
        ...
        self.dw_conv = nn.Conv2d(in_channels, in_channels, kernel_size=kernel_size, padding=kernel_size // 2,
                                 groups=in_channels)
...
class MDFormer(nn.Module):
    def __init__(self, in_channels, out_channels):
        super(MDFormer, self).__init__()
        # Convolution Embedding part
        self.conv_embedding = nn.Conv2d(in_channels, out_channels, kernel_size=3, padding=1)
        self.bn = nn.BatchNorm2d(out_channels)

        # Depthwise Convolutions (only 3x3 and 5x5)
        self.dw3x3 = DWConv(out_channels // 2, 3)  # Channel split, half for 3x3
        self.dw5x5 = DWConv(out_channels // 2, 5)  # Channel split, half for 5x5

        # Pointwise convolutions
        self.pw_conv1 = nn.Conv2d(out_channels, out_channels, kernel_size=1)
        self.pw_conv2 = nn.Conv2d(out_channels, out_channels,
                                  kernel_size=1)  # Restore original channel size after concat

        # Add extra layers as per your request
        self.extra_pw_conv1 = nn.Conv2d(out_channels * 2, out_channels, kernel_size=1)  # First 1x1 conv after res2
        self.extra_dw3x3 = DWConv(out_channels, 3)  # 3x3 depthwise conv
        self.extra_pw_conv2 = nn.Conv2d(out_channels, out_channels, kernel_size=1)  # Another 1x1 conv

        # Final 1x1 convolution to ensure the output has the same channels as input
        self.final_conv = nn.Conv2d(out_channels * 2, in_channels,
                                    kernel_size=1)  # Ensure output matches input channels

        # Residual connections (identity layers if channels are consistent)
    ...
    def forward(self, x):
        ...
        # Channel split
        c = x.shape[1] // 2  # Split channels in half
        x1, x2 = torch.split(x, c, dim=1)  # x1 for 3x3, x2 for 5x5

        # Depthwise convolutions after channel split
        dw3x3_out = self.dw3x3(x1)
        dw5x5_out = self.dw5x5(x2)

        # Concatenate the depthwise convolution outputs
        x = torch.cat([dw3x3_out, dw5x5_out], dim=1)

        # Concatenate with res1 (first residual connection)
        x = torch.cat([x, res1], dim=1)

        # Save this as res2 (second residual connection)
        res2 = x

        # Pointwise convolution after concat with res1
        x = self.pw_conv2(x)

        # Additional convolutions after res2 as per your request
        x = self.extra_pw_conv1(x)  # 1x1 conv after res2
        x = self.extra_dw3x3(x)  # 3x3 depthwise conv
        x = self.extra_pw_conv2(x)  # Another 1x1 conv

        # Concatenate x with res2
        x = torch.cat([x, res2], dim=1)

        # Final 1x1 conv to match input channels
