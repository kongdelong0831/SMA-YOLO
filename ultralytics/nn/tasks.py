...
from ultralytics.nn.modules.V11_block import C3k2, C2PSA
try:
    import thop
except ImportError:
    thop = None
...
class BaseModel(nn.Module):
    ...
    def _apply(self, fn):
        ...
        if isinstance(m, (Detect,Detect_ATDADH, Segment,)):
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolov8n.yaml", ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        # Build strides
        m = self.model[-1]  # Detect()
        if isinstance(m, (Detect, Detect_ATDADH,)):
            s = 640  # 2x min stride
            m.inplace = self.inplace


        # 定义三个卷积层，每个输入通道数为3，输出通道数为8
        self.conv1 = nn.Conv2d(in_channels=3, out_channels=8, kernel_size=3, padding=1)
        self.conv2 = nn.Conv2d(in_channels=3, out_channels=8, kernel_size=3, padding=1)
        self.conv3 = nn.Conv2d(in_channels=3, out_channels=8, kernel_size=3, padding=1)
        # Init weights, biases
...
def attempt_load_weights(weights, device=None, inplace=True, fuse=False):
    ...
    for m in ensemble.modules():
        t = type(m)
        if t in (nn.Hardswish, nn.LeakyReLU, nn.ReLU, nn.ReLU6, nn.SiLU, Detect, Detect_ATDADH, Segment):
            m.inplace = inplace
...
def parse_model(d, ch, verbose=True, warehouse_manager=None):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        elif m in (Detect, Detect_ATDADH, Segment):
            args.append([ch[x] for x in f])
            if m in (Segment):
                args[2] = make_divisible(min(args[2], max_channels) * width, 8)

            if m in (Detect_ATDADH):
